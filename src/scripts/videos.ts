...
async function loadVideosPayload(apiKey: string): Promise<VideosPayload> {
	const [mainResult, vodResult, podcastResult, twitchResult] =
	...
	return {
		...
		twitchLive:
			twitchResult.status === "fulfilled" ? twitchResult.value : false,
...
function renderPayload(
	...
	if (mainGrid) {
		mainGrid.innerHTML = payload.mainVideos.length
			? payload.mainVideos
					.map((video) => renderVideoCard(video, "main", lang, copy))
					.join("")
	...
	if (vodSection && vodGrid) {
		if (payload.vodVideos.length) {
			...
			vodGrid.innerHTML = payload.vodVideos
				.map((video) => renderVideoCard(video, "vod", lang, copy))
				.join("");
		} else {
...
function renderVideoCard(
	...
	const titleHoverClass =
		variant === "main" ? "group-hover:text-primary" : "group-hover:text-accent";
	const badgeMarkup =
		variant === "main"
			? `
...
function readCopy(root: HTMLElement): CopySet {
	return {
		errorText:
			root.dataset.errorText || "No se pudieron cargar los videos ahora mismo.",
		emptyText:
			root.dataset.emptyText || "No hay videos disponibles ahora mismo.",
