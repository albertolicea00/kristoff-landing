			...
			try {
				...
				const videosTab = tabs?.find(
					(t: any) =>
						...
						t.tabRenderer?.content?.sectionListRenderer
				);
				...
				for (const item of items) {
					...
					if (video.upcomingEventData?.startTime) {
						const date = new Date(
							parseInt(video.upcomingEventData.startTime) * 1000
						);
				...
				console.warn(
					"Failed to parse ytInitialData for " +
						channelLabel +
			...
			for (let i = 0; i < Math.min(ids.length, maxResults * 3); i++) {
				...
				videos.push({
					...
					title: decodeHTMLEntities(
						titles[videos.length] || `Video de ${channelLabel}`
					),
					time: "",
...
// 				`https://www.youtube.com/oembed?url=https://www.youtube.com/shorts/${video.id}&format=json`
// 			);
...
export async function fetchLatestVideos() {
	try {
		const [mainVideos, vodVideos, podcastVideos] = await Promise.all([
			scrapeChannelVideos(
				"https://www.youtube.com/@Kristoff9/videos",
				"Kristoff",
				3
			),
			scrapeChannelVideos(
				"https://www.youtube.com/@Kristoffvod/videos",
				"Kristoff TV",
				3
			),
			scrapeChannelVideos(
				"https://www.youtube.com/@EstoNoEsUnPodcastMiami/videos",
				"Podcast",
				1
			),
		]);
