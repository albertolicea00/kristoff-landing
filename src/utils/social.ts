...
export async function getSocialStats() {
	...
	try {
		...
		const subMatch = ytHtml.match(
			/"subscriberCountText":\s*\{[^}]*"simpleText":\s*"([^"]+)"/
		);
		if (subMatch) {
			stats.youtube =
				subMatch[1].replace(" subscribers", "").replace(" suscriptores", "") +
				"+";
		} else {
			// Alternative pattern
			const subMatch2 = ytHtml.match(
				/"subscriberCountText":\{"accessibility":\{"accessibilityData":\{"label":"([^"]+)"/
			);
			if (subMatch2) {
				const cleanSub = subMatch2[1]
